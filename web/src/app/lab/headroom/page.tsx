    ...
    <>
      ...
      <Section eyebrow="Lab · technical verification" title="Token compression reproduction">
        <Verdict label={hr.verdict.replaceAll("_", " ")} kind="mixed"
          sub={<>
            ...
            Probe retention <span className="mono text-text">{hr.retention.toFixed(4)}</span>,{" "}
            <span className="mono text-text">{hr.errors}</span> transformation errors —
      ...
      <Section eyebrow="01" title="The published claims, verbatim"
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {hr.sourceClaims.map((c) => (
            <blockquote key={c.id} className="panel p-5">
              ...
              <footer className="mono mt-3 text-[11.5px] text-faint">
                {c.id} — {c.source}
              </footer>
            </blockquote>
          ))}
        ...
        <p className="body mt-5 max-w-prose">
          The repository description says <strong className="text-text">20%</strong> for
          coding agents; the README says{" "}
          <strong className="text-text">15–20%</strong>. Where a published range and a
          ...
          <table className="text-[12.5px]">
            ...
            <tbody>
              {([["Structured JSON", json], ["Coding context", coding], ["Agent context", agent]] as const)
                .map(([label, d]) => (
                <tr key={label} className="border-b border-lineSoft last:border-b-0">
                  <td className="px-4 py-2.5 font-semibold text-text">{label}</td>
                  <td className="mono px-4 py-2.5 text-dim">{d.vsRaw.n}</td>
                  <td className="mono px-4 py-2.5 text-dim">
                    <span className="text-text">{d.vsRaw.median?.toFixed(2)}%</span>{" "}
                    ({d.vsRaw.p25?.toFixed(2)} · {d.vsRaw.p75?.toFixed(2)})
                  </td>
                  <td className="mono px-4 py-2.5 text-dim">
                    <span className="text-text">{d.vsMinified.median?.toFixed(2)}%</span>{" "}
                    ({d.vsMinified.p25?.toFixed(2)} · {d.vsMinified.p75?.toFixed(2)})
                  </td>
                  <td className="mono px-4 py-2.5 text-dim">{d.minifyOnlyVsRaw.median?.toFixed(2)}%</td>
                  <td className="mono px-4 py-2.5 text-dim">{d.retention.toFixed(4)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="text-[13px]">
            ...
            <tbody>
              {Object.entries(hr.claims).map(([id, c]) => (
                <tr key={id} className="border-b border-lineSoft align-top last:border-b-0">
                  <td className="px-4 py-2.5">
                    <span className="mono text-[11.5px] text-faint">{id}</span>
                    <span className="ml-2 text-text">{CLAIM_TEXT[id]}</span>
                  </td>
                  <td className="mono whitespace-nowrap px-4 py-2.5 text-dim">{c.threshold}</td>
                  <td className="mono whitespace-nowrap px-4 py-2.5 text-dim">
                    {c.value === null ? "—" : id === "CLAIM-C" ? c.value.toFixed(4) : `${c.value.toFixed(2)}%`}
                  </td>
                  <td className={`mono whitespace-nowrap px-4 py-2.5 text-[11.5px] ${
                    c.supported ? "text-signal" : "text-absent"}`}>
      ...
      <Section eyebrow="05" title="The baseline is part of the claim">
        <Reveal>
          <div className="panel-raised border-l-4 border-l-signal p-6">
            <p className="body max-w-prose text-[16px]">
              For structured JSON, headroom saves a median{" "}
              <span className="mono text-text">{json.vsRaw.median.toFixed(2)}%</span>{" "}
              against pretty-printed input — but trivial whitespace minification alone
              saves <span className="mono text-text">{json.minifyOnlyVsRaw.median.toFixed(2)}%</span>{" "}
              (median) on the same samples. Measured against minified input, headroom’s own
              contribution is a median{" "}
              <span className="mono text-text">{json.vsMinified.median.toFixed(2)}%</span>.
            </p>
            <p className="body mt-4 max-w-prose text-[16px]">
              ...
              <strong className="text-text">The baseline is part of the claim.</strong>
            </p>
          </div>
        </Reveal>
      </Section>
      ...
      <Section eyebrow="06" title="The coding claim, and three attempts to rescue it"
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          {hr.supplementary.map((sp) => (
            <div key={sp.id} className="panel flex h-full min-w-0 flex-col break-words p-5">
              ...
              <p className="mono mt-3 border-t border-lineSoft pt-3 text-[11.5px] text-claim">
                {sp.conclusion}
              </p>
            </div>
          ))}
        ...
        <p className="body mt-5 max-w-prose">
          ...
          <span className="mono text-text">{coding.vsMinified.median.toFixed(2)}%</span> —
      ...
      <Section eyebrow="07" title="What this result is — and is not">
        ...
        <details className="panel mt-4">
          ...
          <ul className="border-t border-lineSoft px-5 py-3">
            {hr.untested.map((u) => (
