...
function StateBadge({ lead }: { lead: Lead }) {
  ...
  return (
    <span className={`mono inline-flex items-center gap-1.5 rounded border px-2 py-0.5 text-[11px] uppercase tracking-wider ${
      intro ? "border-signal/40 bg-signal/10 text-signal"
  ...
  return (
    <Reveal>
      <article className="panel overflow-hidden" aria-labelledby={`lead-${index}`}>
        <header className="border-b border-line px-5 py-4 sm:px-7 sm:py-5">
        ...
        <dl className="grid gap-x-8 gap-y-5 px-5 py-5 sm:px-7 sm:py-6 lg:grid-cols-2">
          ...
          <Field label="Actual artifact">
            <span className="mono break-words text-[13px] text-dim">{lead.artifact}</span>
          </Field>
        ...
        <div className="grid gap-px border-t border-line bg-line sm:grid-cols-2">
          <div className="bg-raised px-5 py-4 sm:px-7">
        ...
        <p className="mono border-t border-line px-5 py-3 text-[11.5px] text-faint sm:px-7">
    ...
    <>
      <Section eyebrow="Source → output" title="Current 3"
        ...
        <p className="meta -mt-2 max-w-prose">
          ...
          <span className="mono text-text">{r.introQueue.count}</span>. No introduction has
